...
const Signup = ({ toggelAuthType }: { toggelAuthType: () => void }) => {
	const [actionState, formAction] = useFormState(SignupAction, signupInitialState);

	return (
		<motion.section
			className="w-[90%] md:w-[40%] mx-auto h-full flex items-center origin-right justify-center flex-col absolute max-md:right-1/2 max-md:!translate-x-1/2 md:left-[40px]"
			...
			<main className="w-full">
				<form action={formAction} className="w-full flex flex-col gap-4 transition-all">
					...
					<InputGroup
						...
						animationDelay={actionState.isInfoError && actionState.info.username ? 0.2 : 0}
					/>
					...
					<InputGroup
						...
						animationDelay={
							actionState.isInfoError && actionState.info.email && actionState.info.username
								? 0.45
								: actionState.info.email || actionState.info.username
									? 0.2
									: 0
						}
					/>
